	...
	function setupStatusKey() {
		...
		if (typeof data.used_buffs != "undefined") {
			for(statusKey in data.used_buffs) {
				$('.status-key ul').append('<li><div class="buff '+statusKey+'"></div><div>'+data.used_buffs[statusKey]+'</div></li>');
			}
		}
		if (typeof data.used_debuffs != "undefined") {
			for(statusKey in data.used_debuffs) {
				$('.status-key ul').append('<li><div class="debuff '+statusKey+'"></div><div>'+data.used_debuffs[statusKey]+'</div></li>');
			}
		}
		if (typeof data.used_statuses != "undefined") {
			for(statusKey in data.used_statuses) {
				$('.status-key ul').append('<li><div class="status '+statusKey+'"></div><div>'+data.used_statuses[statusKey]+'</div></li>');
			}
		}
	}
